...
g = .GlobalEnv
source(file = 'script-disc-sheet-ORIG.R')  
@
...
{\Sexpr{g$tlim[2]}}, parabéns por investir na pessoa mais importante do mundo - você.
...
\node[midway,fill=vermelho,text width=130mm]{%
 {\Large\headingfont\filcenter Conquistador(a):}\\
\vspace{5mm}
- Competitivo, pragmático, realista. Gosta de vencer.\\
- Pontos fortes: Persuasivo, confiante, objetivo (vai direto ao ponto)\\
- Pontos fracos: Autoritário, centralizador, pode ser visto como insensível

};
\node at (8.7,0){\Huge\textcolor{vermelho}{{\Sexpr{print(g$disc$nivel[1])}{\%}} } };

\end{tikzpicture}

\begin{tikzpicture}
\node[fill=laranja,text width=130mm]{%
{\Large\headingfont\filcenter Diplomata:}\\
\vspace{5mm}
- Carismático, encantador, idealista. Gosta de atenção.\\
- Pontos fortes: Alegre, criativo, convincente\\
- Pontos fracos: Dificuldades na organização e disciplina. Problemas ao priorizar suas necessidades.

};
\node at (8.7,0){\Huge\textcolor{laranja}{{\Sexpr{print(g$disc$nivel[2])}{\%}} } };

\end{tikzpicture}

\begin{tikzpicture}
\node[fill=verde,text width=130mm]{%
{\Large\headingfont\filcenter Sacerdote:}\\
\vspace{5mm}
- Leal, companheiro, pacificador. Gosta de ajudar os outros.\\
- Pontos fortes: Bom ouvinte, paciente, adaptável a qualquer situação\\
- Pontos fracos: Evita conflitos, pode parecer desinteressado, tem dificuldade em tomar decisões

};
\node at (8.7,0){\Huge\textcolor{verde}{{\Sexpr{print(g$disc$nivel[3])}{\%}} } };

\end{tikzpicture}

\begin{tikzpicture}
\node[fill=azul,text width=130mm]{%
{\Large\headingfont\filcenter Cientista:}\\
\vspace{5mm}
- Especialista, busca entender a fundo, observador. Gosta de organização e perfeição.\\
- Pontos fortes: disciplinado, atento, curioso e obstinado\\
- Pontos fracos: pouca flexibilidade para coisas novas, dificuldades em se comunicar com clareza

};
\node at (8.7,0){\Huge\textcolor{azul}{{\Sexpr{print(g$disc$nivel[4])}{\%}} } };
...
\def\perfilprincipal{\Sexpr{g$disc_ord$perfil[1]}}
\def\perfilsecundario{\Sexpr{g$disc_ord$perfil[2]}}


\ifnum\perfilprincipal=1
{\Sexpr{g$tlim[2]}}{, }{\input{txt/d1.txt}}
\fi

\ifnum\perfilprincipal=2
{\Sexpr{g$tlim[2]}}{, }{\input{txt/i1.txt}}
\fi

\ifnum\perfilprincipal=3
{\Sexpr{g$tlim[2]}}{, }{\input{txt/s1.txt}}
\fi

\ifnum\perfilprincipal=4
{\Sexpr{g$tlim[2]}}{, }{\input{txt/c1.txt}}
\fi
